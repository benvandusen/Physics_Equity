...
#pool_and_cov <- function(x,y){ #I'm just not going ot make this as a function for now since I am only going to run it once and it might be really tricky with differing sizes and positions for the variables
  ...
  sxp3 <- summary(glht(gain6[[i]], linfct=contrast_forms_mod6)) #specifically for post3
  covp3 <- vcov(glht(gain6[[i]], linfct=contrast_forms_mod6))
  data.frame(imp=i, 
...
rat_maj_fem <- p3est$Q[1]/p3est$Q[2]
rat_maj_mal <- p3est$Q[3]/p3est$Q[4]
rat_urm_fem <- p3est$Q[5]/p3est$Q[6]
rat_urm_mal <- p3est$Q[7]/p3est$Q[8]

temp <- get.est[get.est$group=="Majority Female Lecture",]
cov_maj_fem <- mean(temp$cov.Majority.Female.Collab)

temp <- get.est[get.est$group=="Majority Male Lecture",]
cov_maj_mal <- mean(temp$cov.Majority.Male.Collab)

temp <- get.est[get.est$group=="URM Female Lecture",]
cov_urm_fem <- mean(temp$cov.URM.Female.Collab)

temp <- get.est[get.est$group=="URM Male Lecture",]
cov_urm_mal <- mean(temp$cov.URM.Male.Collab)


 
se_rat_maj_fem <- sqrt( (rat_maj_fem)^2 *  ((p3est$T[2])/(p3est$Q[2]^2)  +  (p3est$T[1])/(p3est$Q[1]^2)  -2*(cov_maj_fem/((p3est$Q[1])*(p3est$Q[2])) )))

se_rat_maj_mal <- sqrt( (rat_maj_mal)^2 *  ((p3est$T[4])/(p3est$Q[4]^2)  +  (p3est$T[3])/(p3est$Q[3]^2)  -2*(cov_maj_mal/((p3est$Q[3])*(p3est$Q[4])) )))

se_rat_urm_fem <- sqrt( (rat_urm_fem)^2 *  ((p3est$T[6])/(p3est$Q[6]^2)  +  (p3est$T[5])/(p3est$Q[5]^2)  -2*(cov_urm_fem/((p3est$Q[5])*(p3est$Q[6])) )))

se_rat_urm_mal <- sqrt( (rat_urm_mal)^2 *  ((p3est$T[8])/(p3est$Q[8]^2)  +  (p3est$T[7])/(p3est$Q[7]^2)  -2*(cov_urm_mal/((p3est$Q[7])*(p3est$Q[8])) )))


rat_self_table <- data.frame(group=c("Majority Female","Majority Male","URM Female","URM Male"),
                             ratio = c(p3est$Q[1]/p3est$Q[2],p3est$Q[3]/p3est$Q[4],p3est$Q[5]/p3est$Q[6], p3est$Q[7]/p3est$Q[8]),
                             se = c(sqrt( (rat_maj_fem)^2 *  ((p3est$T[2])/(p3est$Q[2]^2)  +  (p3est$T[1])/(p3est$Q[1]^2)  -2*(cov_maj_fem/((p3est$Q[1])*(p3est$Q[2])) ))),
                                    sqrt( (rat_maj_mal)^2 *  ((p3est$T[4])/(p3est$Q[4]^2)  +  (p3est$T[3])/(p3est$Q[3]^2)  -2*(cov_maj_mal/((p3est$Q[3])*(p3est$Q[4])) ))),
                                    sqrt( (rat_urm_fem)^2 *  ((p3est$T[6])/(p3est$Q[6]^2)  +  (p3est$T[5])/(p3est$Q[5]^2)  -2*(cov_urm_fem/((p3est$Q[5])*(p3est$Q[6])) ))),
                                    sqrt( (rat_urm_mal)^2 *  ((p3est$T[8])/(p3est$Q[8]^2)  +  (p3est$T[7])/(p3est$Q[7]^2)  -2*(cov_urm_mal/((p3est$Q[7])*(p3est$Q[8])) )))))

```

```{r}
ggplot(rat_self_table, aes(y=ratio, x=group)) +geom_bar(stat="identity", fill='#fc8d62') +
  geom_errorbar(aes( ymax= ratio+1.96*se, ymin=ratio-1.96*se, width=0.5)) +
  theme(legend.position = "bottom", axis.title.x=element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),text = element_text(size=16, color = "black")) +
...
Ben's Models
```{r }
mod1 <- gain~1 + (1|course_id)
mod2 <- gain~1 + stud_pre_cent + (1|course_id)
mod3 <- gain~1 + stud_pre_cent + class_pre_cent + (1|course_id)
mod4 <- gain~1 + stud_pre_cent + race_URM + gend_URM + (1|course_id)
mod5 <- gain~1 + stud_pre_cent + race_URM*gend_URM + (1|course_id)
mod6 <- gain~1 + race_URM + gend_URM + stud_pre_cent + coll + (1|course_id)
mod7 <- gain~1 +race_URM*coll + gend_URM*coll+ stud_pre_cent + (1|course_id)
mod8 <- gain~1 +race_URM + gend_URM + stud_pre_cent + coll + (1 + stud_pre_cent + gend_URM + race_URM |course_id)
```
...
gain4 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + retake + course_pre + (1|course_id))})
gain5 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + retake + FMCE + (1|course_id))})
gain6 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + retake + female + (1|course_id))})
gain7 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + retake + female + asian + black + Hispanic + (1|course_id))})
gain8 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + retake + female + asian + black + Hispanic + lec + (1|course_id))})
gain9 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + retake + female*(asian + black + other + Hispanic) + lec + (1|course_id))})
gain10 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + retake + female*(asian + black + other + Hispanic) + female*lec +  (1|course_id))})
gain11 <- with(MIdata,{lmer(gain~1 + lec*female*(asian + black + other + Hispanic) + stud_pre_cent + retake + (1|course_id))})
gain12 <- with(MIdata,{lmer(gain~1 + lec*female*(asian + black + other + Hispanic) + stud_pre_cent + retake + (1 + stud_pre_cent + gend_URM + race_URM |course_id))})

#Old models
#gain1 <- with(MIdata,{lmer(gain~1 + (1|course_id))})
#gain2 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + (1|course_id))})
#gain3 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + class_pre_cent + (1|course_id))})
#gain4 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + race_URM + gend_URM + (1|course_id))})
#gain5 <- with(MIdata,{lmer(gain~1 + stud_pre_cent + race_URM*gend_URM + (1|course_id))})
#gain6 <- with(MIdata,{lmer(gain~1 + race_URM + gend_URM + stud_pre_cent + coll + (1|course_id))})
#gain7 <- with(MIdata,{lmer(gain~1 + (race_URM + gend_URM)*coll + stud_pre_cent + (1|course_id))})
#gain8 <- with(MIdata,{lmer(gain~1 +race_URM + gend_URM + stud_pre_cent + coll + (1 + stud_pre_cent + gend_URM + race_URM |course_id))})
```
...
This produces the estimates for gain6
```{r}
WML = c(1,0,0,0,0)
WMC = c(1,0,0,0,1)
BML = c(1,1,0,0,0)
...
contrast_forms_mod6 <- rbind('Majority Male Lecture'=WML, 'Majority Male Collab'=WMC, 
                          'URM Male Lecture'=BML, 'URM Male Collab'=BMC, 
                          'Majority Female Lecture'=WFL,'Majority Female Collab'=WFC, 
                          'URM Female Lecture'=BFL,'URM Female Collab'=BFC)
```
